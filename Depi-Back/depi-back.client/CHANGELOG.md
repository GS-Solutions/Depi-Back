...
- Create Angular project with ng: `ng new depi-back.client --defaults --skip-install --skip-git `.
...
- Create project file (`depi-back.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `depi-back.client.esproj`).
